...
public class ForgeMixinConfigPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        if (mixinClassName.equals("io.github.breezethecat.hextest.forge.mixin.MixinDatagenMain")) {
            var shouldApply = System.getProperty("hextest.apply-datagen-mixin", "false").equals("true");
            if (shouldApply) {
                Hextest.LOGGER.warn("Applying scuffed datagen mixin. This should not happen if not running datagen!");
            }
            return shouldApply;
        }
        return true;
